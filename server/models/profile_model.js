...
const createResume = async (userId, resume) => {
    ...
    try {
        ...
        const profile = {
            ...
            birthday: resume.birthday ? resume.birthday : null,
            phone: resume.phone,
        ...
        if (resume.projectTitle) {
            ...
            await conn.query("INSERT INTO mayones.resume_projects (resume_id, project_title, project_link, project_intro, project_image) VALUES ? ", [
                projects,
            ]);
        }
        ...
        if (resume.experienceName) {
            for (let i = 0; i < resume.experienceName.length; i++) {
                experience.push([
                    result.insertId,
                    resume.experienceName[i],
                    resume.experienceCompanyName[i],
                    resume.experienceTimeStart[i],
                    resume.experienceTimeEnd[i],
                    resume.experienceInfo[i],
                ]);
            }
            await conn.query(
                "INSERT INTO mayones.resume_experience(resume_id, experience_title, experience_org, experience_start, experience_end, experience_intro) VALUES ? ",
                [experience]
            );
        }
        ...
        if (resume.educationName) {
            for (let i = 0; i < resume.educationName.length; i++) {
                education.push([
                    result.insertId,
                    resume.educationName[i],
                    resume.educationDepartment[i],
                    resume.educationDegree[i],
                    resume.educationTimeStart[i],
                    resume.educationTimeEnd[i],
                ]);
            }
            await conn.query(
                "INSERT INTO mayones.resume_education (resume_id, education_title, education_department, education_degree, education_start, education_end) VALUES ? ",
                [education]
            );
        }
...
const userUpdateResume = async (userId, resume) => {
    ...
    try {
        ...
        if (resume.projectId[0]) {
                ...
                if (resume.projectId[i] == parseInt(resume.projectId[i])) {
                    if (resume.projectImageSrc[i][resume.projectId[i]] === "upload") {
                        projects.push([
                            resume.projectId[i],
                            resume.resumeId[0],
                            resume.projectTitle[i],
                            resume.projectLink[i],
                            resume.projectInfo[i],
                            resume.projectImage.shift(),
                        ]);
                    } else {
                        projects.push([
                            resume.projectId[i],
                            ...
                            resume.projectLink[i],
                            resume.projectInfo[i],
                            resume.projectImageSrc[i][resume.projectId[i]],
                        ]);
                    }
                } else {
                    projects.push([
            ...
            await conn.query(
                `INSERT INTO mayones.resume_projects 
        ...
        if (resume.experienceId[0]) {
            ...
            await conn.query(
                `INSERT INTO mayones.resume_experience 
            ...
            experience_intro=VALUES(experience_intro)`,
                [experience]
            );
        }
        ...
        if (resume.educationId[0]) {
            ...
            await conn.query(
                `INSERT INTO mayones.resume_education 
            ...
            education_end=VALUES(education_end)`,
                [education]
            );
        }
        await conn.query("COMMIT");
